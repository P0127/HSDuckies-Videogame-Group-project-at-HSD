...
func spawn_mob():
	...
	if (which_mob > 0.2): #80% chance for meelee mob
		...
		if(MapFloor.get_cell_source_id(MapFloor.local_to_map(MobSpawningPath.global_position)) == 0):
		...
		if(MapFloor.get_cell_source_id(MapFloor.local_to_map(MobSpawningPath.global_position)) == 0):
			...
			spawn_mob()
			#print($Test_Tilemap/Boden.get_cell_source_id(MobSpawningPath.global_position)) #testing
