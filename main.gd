	...
	if(current_mob_amount <= MOB_LIMIT):
		if (which_mob > 0.2): #80% chance for meelee mob 
			...
			if(MapFloor.spawncheck(MobSpawningPath.global_position / 3.33)):
				...
				print("meelee")
				print(MapFloor.get_cell_source_id(MapFloor.to_local(MobSpawningPath.global_position)))
				print(MapFloor.local_to_map(MapFloor.to_local(MobSpawningPath.global_position)))
			...
			if(MapFloor.spawncheck(MobSpawningPath.global_position / 3.33)):
				...
				print("ranged:")
				print(MapFloor.get_cell_source_id(MapFloor.to_local(MobSpawningPath.global_position))) #testing
				print(MapFloor.local_to_map(MapFloor.to_local(MobSpawningPath.global_position)))
				spawn_mob()#try again with different position
