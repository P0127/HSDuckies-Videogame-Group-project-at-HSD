...
#will prob change to make unique run aways with each model since doesnt make sense to transform all to one
...
func _physics_process(delta):
	...
	if velocity.length() == 0:
		$AnimatedRangedMobSprite.animation = "front"
	#Flips Animation if walking to the side
	if velocity.x != 0:
		$AnimatedRangedMobSprite.animation = "side"
		# uprightposture
		$AnimatedRangedMobSprite.flip_v = false
		$AnimatedRangedMobSprite.flip_h = velocity.x < 0
